...
applicationStackOutputs=$(aws cloudformation describe-stacks --stack-name ${TEST_APPLICATION_STACK_NAME:-'javascript-appsync-sam-js'} | jq -r '.Stacks[0].Outputs')
...
cognitoStackOutputs=$(aws cloudformation describe-stacks --stack-name ${TEST_COGNITO_STACK_NAME:-'serverless-api-cognito'} | jq -r '.Stacks[0].Outputs')
...
result=$(aws cognito-idp list-users --user-pool-id $userPool --filter "email = \"$regularUserName\""  | jq -r '.Users[].Username')
if ! [ -z "$result" ]; then
...
# --- Deleting user if it already exists ...
result=$(aws cognito-idp list-users --user-pool-id $userPool --filter "email = \"$adminUserName\""  | jq -r '.Users[].Username')
if ! [ -z "$result" ]; then
