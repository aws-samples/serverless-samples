						...
						if (recordType.equalsIgnoreCase("heartbeat")) {
							...
							if (!(databaseActivityEvent.getDbUserName().strip().equalsIgnoreCase("rdsadmin"))) {
								...
								try {
									...
									logger.log(correlationId + "Successfully wrote event to S3");
								} catch (Exception e) {
									// Log error with context but without sensitive data
									logger.log(correlationId + "ERROR: Failed to write event to S3 after retries. " +
										"Error type: " + e.getClass().getSimpleName() + 
										", Message: " + e.getMessage());
									// Re-throw to trigger Lambda retry and DLQ if configured
									throw new RuntimeException("S3 write failed for event", e);
								}
							} else {
