...
def test_access_to_the_locations_without_authentication(global_config):
    response = requests.get(global_config["api_endpoint"] + '/locations')
    assert response.status_code == 401


def test_get_list_of_locations_by_regular_user_without_api_key(global_config):
    response = requests.get(
        global_config["api_endpoint"] + '/locations',
...
def test_get_list_of_locations_by_regular_user(global_config):
    response = requests.get(
        global_config["api_endpoint"] + '/locations',
        headers={'Authorization': global_config["regularUserIdToken"],
                 'X-Api-Key': global_config["enterpriseUsagePlanApiKeyValue"]}
    )
...
def test_get_location_by_regular_user(global_config):
    response = requests.get(
        global_config["api_endpoint"] + f'/locations/{new_location_id}',
        headers={'Authorization': global_config["regularUserIdToken"],
                 'X-Api-Key': global_config["enterpriseUsagePlanApiKeyValue"]}
    )
    assert response.status_code == 200
...
def test_access_to_the_resources_without_authentication(global_config):
    response = requests.get(global_config["api_endpoint"] + f'/locations/{new_location_id}/resources')
    assert response.status_code == 401


def test_get_list_of_resources_by_regular_user(global_config):
    response = requests.get(
        global_config["api_endpoint"] + f'/locations/{new_location_id}/resources',
        headers={'Authorization': global_config["regularUserIdToken"],
                 'X-Api-Key': global_config["enterpriseUsagePlanApiKeyValue"]}
    )
    assert response.status_code == 200
...
def test_get_resource_by_regular_user(global_config):
    response = requests.get(
        global_config["api_endpoint"] + f'/locations/{new_location_id}/resources/{new_resource_id}',
        headers={'Authorization': global_config["regularUserIdToken"],
                 'X-Api-Key': global_config["enterpriseUsagePlanApiKeyValue"]}
    )
    assert response.status_code == 200
...
def test_access_to_the_bookings_without_authentication(global_config):
    response = requests.get(
        global_config["api_endpoint"] + f'/locations/{new_location_id}/resources/{new_resource_id}/bookings')
    assert response.status_code == 401
...
def test_deny_put_booking_for_somebody_else(global_config):
    response = requests.put(
        global_config["api_endpoint"] + f'/users/{global_config["adminUserSub"]}/bookings',
...
def test_deny_put_booking_for_somebody_else_by_administrative_user(global_config):
    response = requests.put(
        global_config["api_endpoint"] + f'/users/{global_config["regularUserSub"]}/bookings',
        data=json.dumps(new_booking),
        headers={'Authorization': global_config["adminUserIdToken"],
                 'X-Api-Key': global_config["enterpriseUsagePlanApiKeyValue"],
                 'Content-Type': 'application/json'}
...
def test_get_details_of_your_booking(global_config):
    response = requests.get(
        global_config["api_endpoint"] + f'/users/{global_config["regularUserSub"]}/bookings/{new_booking_id}',
        headers={'Authorization': global_config["regularUserIdToken"],
                 'X-Api-Key': global_config["enterpriseUsagePlanApiKeyValue"]}
    )
    assert response.status_code == 200
