# Create Resources with AWS SAM

This project contains source code and supporting files for a serverless application that you can deploy with the AWS Serverless Application Model (AWS SAM) command line interface (CLI). This SAM application will create an AWS Lambda function that uses the SQS queue as its event source. Notice in particular the AWS CloudFormation dynamic reference contained in the template.yaml file that retrieves the SQS queue arn stored in SSM Parameter Store.
...
Navigate to the `terraform-sam-integration/sam` directory of this project.
...
SAM has the capability to synchronize your local code with your cloud environment to speed up developer iteration cycles. 

**[Watch this YouTube video](https://www.youtube.com/watch?v=DDhOa4ekn80&ab_channel=ServerlessLand)** for an explanation and demo. 

The video is 22 minutes long, but understanding SAM sync could save you hours of time. 
...
```bash
sam logs -n SQSPayloadLogger --stack-name sam-app --tail
```
...
To delete the sample application and the bucket that you created:
