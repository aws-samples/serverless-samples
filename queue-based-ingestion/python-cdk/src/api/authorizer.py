...
def validate_token(token, region):
    ...
    if is_cold_start:
        ...
        keys_url_response = requests.get(keys_url)        
        keys = json.loads(keys_url_response.text) ['keys']
        is_cold_start = False
...
def lambda_handler(event, context):
    ...
    policy.allow_method(HttpVerb.POST, "job-request")
    policy.allow_method(HttpVerb.GET, "job-request/*")
    policy.allow_method(HttpVerb.POST, "submit-job-request/*")
