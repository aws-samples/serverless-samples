...
## Overview
 Implementation of this architecture deploys infrastructure and API stacks in two AWS regions. It  walks through Amazon Route 53 manual configuration steps that can be tailored to fit your routing requirements. 
...
This architecture uses the following resources in each region:
 ...
 - Amazon Virtual Private Cloud (Amazon VPC) with two private subnets, used to deploy VPC Endpoint and Network Load Balancers (NLB) used to access API Gateway; same VPC can be used to deploy client applications that need access to the private API.
...
--region us-east-1
```
Repeat this process in the us-west-2 region and take note of both certificate ARNs as you will need them later.
...
At this point, you should have the VPCs in the two regions, with VPC Endpoints to API Gateway, interconnected using Transit Gateways with Inter-Region Peering. 
...
To install API Gateway in both regions and set up private custom domains, run the following steps:
 - Navigate to the repository root directory where api.yaml is located
...
At this point, you should have the stacks with the API Gateway, Network Load Balancer and CloudWatch Alarms deployed to the two regions and ready for the next step.

## Configure Route 53
Once the infrastructure and the API stacks are deployed in both regions, create health checks to use them in Route 53:
...
- Create private Route 53 hosted zone, e.g. internal.example.com, and associate with the VPCs where the client applications that access private APIs reside:
...
dig +short private.internal.com
 ```
Response should contain IP addresses of the Network Load Balancer in one of the VPCs:
``` bash
10.2.2.188
10.2.1.211
 ```
- After DNS resolution is verified, execute the following command in each of the VPCs (use domain name that matches your configuration):
``` bash
curl -s -XGET https://private.internal.example.com/demo
 ```
Response should include event data as Lambda function received it.
- To simulate outage, navigate to Route 53 in the management console, pick health check that corresponds to one of the regions where you received response from, and invert health status:
 ![Route 53 healthcheck](assets/healthcheck_inverted.png)
- After few minutes try the same DNS resolution and API response validation steps. This time you should see that all the requests are routed to the remaining healthy API stack.
